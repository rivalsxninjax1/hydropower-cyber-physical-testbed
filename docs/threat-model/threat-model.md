...
| `experiments/01-modbus-control/` | [T0855](https://attack.mitre.org/techniques/T0855/) | Unauthorized Command Message | Impair Process Control | PLC control register (40007) | Direct Modbus TCP write, no auth | Gate 45%→69%, flow 68.7→105.4 m³/s, RPM 1500→1712, alarm NORMAL→WARNING (measured, 3 runs, bit-identical) | **Phase 11 IDS**: `anomalous_command_value` rule, t+0.00s (vs. t+8.14s for the plant's own alarm system) | **Phase 12 segmentation**: fully blocks this attack at the network layer, confirmed by Experiment 02 |
